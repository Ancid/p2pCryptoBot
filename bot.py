...
#     is_active = await db_check_subscription(message.chat.id)
#     bot.send_message(message.chat.id, MSG_HELLO, reply_markup=markup_actions(is_active))
#
#
...
#             await show_offers(call.message.chat.id, True)
#         if call.data.startswith('same_check:same'):
#             await show_offers(call.message.chat.id, False, True)
...
#
#
# async def show_offers(chat_id, paginated=False, reset_page=False):
#     if reset_page:
#         db_update_search_page(chat_id, 1)
#     user = db_qget_user(chat_id)
...
#         await log_search(user)
#
#     offers = await get_offers_array(offer_type, payment_method, currency)
#     if offers is None:
#         bot.send_message(
#             chat_id,
...
#             is_active = await db_check_subscription(chat_id)
#             bot.send_message(
#                 chat_id,
#                 MSG_OFFERS,
